...
volatile boolean ALARM2 = false; //CREATING A NEW ALARM ALARM2
...
// After wakeup starts here
  ...
  if (ALARM1 == false) { // If alarm hasn't happened yet, set alarm to first alarm time. 
    DateTime now = rtc.now();
    rtc.setAlarm1Simple(21, 02); // ***SET FIRST CAPTURE TIME HERE IN 24 HR FORMAT AS (HOUR, MINUTES)*** 
  ...
  if (ALARM1 == true && ALARM2 == false) { // If first alarm has already gone off, set alarm to second desired time. 
    DateTime now = rtc.now();
    rtc.setAlarm1Simple(21, 03); // ***SET SECOND CAPTURE TIME HERE IN 24 HR FORMAT AS (HOUR, MINUTES)*** 
    rtc.turnOnAlarm(1); // Turn alarm on
    if (rtc.checkAlarmEnabled(1)) { // If it's enabled, tell me. 
    ...
    if (ALARM1 == false && ALARM2 == true ) { // If second alarm has already gone off, set alarm to third desired time. 
    DateTime now = rtc.now();
    rtc.setAlarm1Simple(21, 04); // ***SET third CAPTURE TIME HERE IN 24 HR FORMAT AS (HOUR, MINUTES)*** 
    rtc.turnOnAlarm(1); // Turn alarm on
    if (rtc.checkAlarmEnabled(1)) { // If it's enabled, tell me. 
      Serial.println("third Alarm Enabled");
      delay(100);
    }
  }


    if (ALARM1 == true && ALARM2 == true ) { // If third alarm has already gone off, set alarm to fourth desired time. 
    DateTime now = rtc.now();
    rtc.setAlarm1Simple(21, 06); // ***SET fourth CAPTURE TIME HERE IN 24 HR FORMAT AS (HOUR, MINUTES)*** 
    rtc.turnOnAlarm(1); // Turn alarm on
    if (rtc.checkAlarmEnabled(1)) { // If it's enabled, tell me. 
      Serial.println("fourth Alarm Enabled");
      delay(100);
    }
  }
...
void awakeNow() { 
  ...
  if (flag == true & counter == 2) { // if counter is two (second wake of the day) then run second alarm routine, then reset flags 
    ...
    ALARM2 = true; //NEXT STATE third alarm
    Serial.println("ALARM1 flag set to false");
    Serial.println("ALARM2 flag set to true");
    //counter = 0; // Reset daily wakeup counter
    //Serial.println("Daily counter reset.");
  }

  if (flag == true & counter == 3) { // if counter is three (third wake of the day) then run third alarm routine, then reset flags 
    Serial.println("I am awake now. Doing third alarm stuff..."); 
    // DO STUFF HERE
    runCamera();
    flag = false; // Reset flag.
    Serial.println("flag reset");
    ALARM1 = true; // Reset flag
    ALARM2 = true;
    Serial.println("ALARM1 flag set to true");
    Serial.println("ALARM2 flag set to true");
    //counter = 0; // Reset daily wakeup counter
  ...
  if (flag == true & counter == 4) { // if counter is three (third wake of the day) then run third alarm routine, then reset flags 
    Serial.println("I am awake now. Doing fourth alarm stuff..."); 
    // DO STUFF HERE
    runCamera();
    flag = false; // Reset flag.
    Serial.println("flag reset");
    ...
    ALARM2 = false;
    Serial.println("ALARM1 flag set to false");
    Serial.println("ALARM2 flag set to false");
    counter = 0; // Reset daily wakeup counter
    Serial.println("Daily counter reset.");
  }
