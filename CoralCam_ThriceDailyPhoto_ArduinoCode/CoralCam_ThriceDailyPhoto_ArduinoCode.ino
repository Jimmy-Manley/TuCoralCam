...
// After wakeup starts here
  ...
  if (ALARM1 == false) { // If alarm hasn't happened yet, set alarm to ALARM1 time. 
    DateTime now = rtc.now();
    rtc.setAlarm1Simple(20, 20); // ***SET FIRST CAPTURE TIME HERE IN 24 HR FORMAT AS (HOUR, MINUTES)*** 
  ...
  if (ALARM1 == true && ALARM2 == false) { // If Alarm1 has already gone off, set alarm to second desired time. 
    DateTime now = rtc.now();
    rtc.setAlarm1Simple(20, 22); // ***SET SECOND CAPTURE TIME HERE IN 24 HR FORMAT AS (HOUR, MINUTES)*** 
    rtc.turnOnAlarm(1); // Turn alarm on
    if (rtc.checkAlarmEnabled(1)) { // If it's enabled, tell me. 
    ...
    if (ALARM1 == false && ALARM2 == true ) { // If Alarm1 has already gone off, set alarm to second desired time. 
    DateTime now = rtc.now();
    rtc.setAlarm1Simple(20, 24); // ***SET SECOND CAPTURE TIME HERE IN 24 HR FORMAT AS (HOUR, MINUTES)*** 
    rtc.turnOnAlarm(1); // Turn alarm on
    if (rtc.checkAlarmEnabled(1)) { // If it's enabled, tell me. 
      Serial.println("Second Alarm Enabled");
      delay(100);
...
//    if (ALARM1 == true && ALARM2 == true ) { // If Alarm1 has already gone off, set alarm to second desired time. 
//    DateTime now = rtc.now();
//    rtc.setAlarm1Simple(20, 26); // ***SET SECOND CAPTURE TIME HERE IN 24 HR FORMAT AS (HOUR, MINUTES)*** 
//    rtc.turnOnAlarm(1); // Turn alarm on
//    if (rtc.checkAlarmEnabled(1)) { // If it's enabled, tell me. 
//      Serial.println("Second Alarm Enabled");
//      delay(100);
